...
**✅ RESOLVED 2026-07-27 (user asked):** root cause was the Authelia secrets directory
...
secrets directory (surgical; file contents untouched). Authelia restarted successfully,
...
(`v20.18.0`, only `v20.20.2` exists) — invisible to thegrid-api's absolute-path deploy script,
...
**2026-08-01 addendum — the runs weren't missing because of Gitea.** The debugging detour
...
4. **Infrastructure fingerprinting** — `BRIEF.md`'s exhaustive port inventory of the
   entire host was trimmed to DeckScout's own 3700-3709 block; Authelia postmortem
...
## 2026-08-09 — Pi instance decommissioned, homelab machinery removed

The TheGrid Pi deployment is decommissioned: pm2 apps deleted, nginx includes
removed, full backup taken first (DB dump + image tar in
`~/deckscout-backups/`). Homelab-only machinery removed from the repo —
`deploy/` (nginx fragments, dnsmasq conf, pm2 ecosystem config, BACKUP.md,
DEPLOY.md), `tools/devhub/` (LAN dev-hub, standalone/no workspace deps),
`issues/` (46 resolved personal bug reports — a SaaS project tracks issues on
GitHub instead), `.gitea/` (Gitea CI, superseded by GitHub Actions), and root
`ecosystem.config.cjs`. `.gitignore` gained a `deploy/` rule (homelab deploy
...
**What landed (five commits):** homelab machinery purged (deploy/, devhub, issues/,
Gitea CI, pm2 config); migrations 020 (BIGINT→UUID owners, user_id on deck_version +
