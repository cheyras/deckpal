...
shots at 390x844 / 744x1133 / 1133x744 in local dev-artifacts/foil-shots/starlight-rework/.
...
local dev-artifacts/foil-shots/anycard/. Typecheck + build green (web + api).
...
(screenshots + diag script output in local dev-artifacts/foil-shots/r0/diffuse/).
...
local dev-artifacts/foil-shots/assignments/): base4-2 cosmos set|high (Chey's card), Fossil
...
local dev-artifacts/foil-shots/r2/ (390px + desktop).
...
Shots: local dev-artifacts/foil-shots/r2b/ · runs: verify-<p>-r2b(-2)/ · jobs:
...
renders in local dev-artifacts/foil-shots/r3-sheen/swap-evidence/):**
...
frames `local dev-artifacts/foil-verify/frames/tint-*-{pre,post}/`, headline shots
`local dev-artifacts/foil-shots/r3-misc/`. Same contract change added **uP4/uP5** param
...
- **The two standing punch items this serves:** (a) SM era has no era-layout rects —
  ...
  in local dev-artifacts/foil-shots/mask-refine/ are the evidence.
...
`local dev-artifacts/foil-shots/closer/`): Adjust window on base1-4 (handles drag,
...
shots (`local dev-artifacts/foil-shots/r4b/`, 3 tilts × desktop + 390px).
...
`local dev-artifacts/foil-shots/canon-preview/`). Regenerate the baked file
...
Curve plot: `local dev-artifacts/foil-shots/r4c/onset-curve-grubbin-before-after.png`
...
`local dev-artifacts/foil-shots/r4d/` (pointerdrag-30px-before-after.png,
...
control pair (AE 0). Shots: `local dev-artifacts/foil-shots/r5/`.
...
`local dev-artifacts/foil-shots/integrate-prov/`.
...
a real assigned card at 390px and desktop in `local dev-artifacts/foil-shots/r7/`;
...
`local dev-artifacts/foil-shots/mask-straighten/`. The rendered foil is near
...
## 2026-08-08 — `edge-trace@1`: his mask is the INTENT, the artwork is the GEOMETRY
**Decided by:** user (redirect), agent (method).

**Chey, in chat, verbatim:** *"I made changes to the tropius mask. Really just 'straighten'
isn't quite enough, really just detecting edges and actually tracing accurately around them
is the real move."*

### The premise shift (this is the whole entry)

`line-snap@1` treats **his strokes as the geometry** and nudges near-axis runs onto printed
lines where the evidence is strong, leaving everything else as drawn. That frame is wrong.
His mask is a **statement of INTENT** — which regions of the card carry foil. The
**ARTWORK holds the true geometry**. So the output boundary is *derived from the card's own
detected edges*, traced accurately — curves, rounded corners, tag ends, panel fillets
included — with his mask used only to decide **which** edge to follow and which side is
inside. Straight-only, evidence-gated nudging is now the fallback, not the method.

**His own correction said the same thing before he did.** His edit of the line-snap output
was 627px (Jaccard 0.9964) — and **114 of them sit in one 40×50px box around the species
strip's swept tail**, which line-snap had squared off by intersecting two lines across it.
He hand-restored a curve the generator structurally could not represent. That is the single
loudest signal the provenance loop has produced, and it is exactly what the loop was for.

### The method (`apps/api/src/foil/edge-trace.ts`, pure, no I/O)

**Di Zenzo colour structure tensor** (Scharr per channel, J = Σ ∇I_c ∇I_cᵀ) → non-maximum
suppression → hysteresis linking. Colour, not luminance: a green printed field against a
silver card border is a large **chroma** step and a small luminance one. The pathological
case is locked by a test: green `(120,190,60)` against silver `(166,166,166)` differ by
**0.3 in luminance** and read **0.13 luminance/px vs 44.3 tensor/px** at the same edge — a
luminance Sobel is simply blind to it, and it is a boundary a foil mask has to follow.

Then **livewire / intelligent scissors**, the classic "trace accurately around the real
edge" algorithm: a distance+index **corridor** around his boundary (nothing beyond
`corridorPx` 5 is reachable — his intent is a hard bound, not a suggestion) → **anchors**
every `anchorSpacingPx` 12 **plus every hard turn in his own line** (`cornerAnchorDeg` 35),
each snapped to the strongest ridge within `anchorSnapPx` 3 → Dijkstra between consecutive
anchors on `wRidge`(1−ridge) + `wLinked` + `wProximity`(d/corridor)^`proximityPower` 2 +
`wDirection` → **sub-pixel refinement** onto the parabolic peak of |∂I/∂n| along the path
normal → **and only then** a straight fit, where the traced path genuinely is straight.
Nothing in the cost function prefers straightness, which is precisely why a fillet, a
rounded tag end and a swept tail come out as themselves.

Refusals, each a param and a report line: no scan ⇒ his mask handed straight back; an
anchor with no ridge ≥ `anchorMinStrength` 12 ⇒ that stretch stays exactly as drawn; a path
under `segmentMinRidge` 10 or detouring past `segmentMaxDetourRatio` 1.6 ⇒ kept; no path in
the corridor ⇒ kept; every move capped by `corridorPx`.

### Two traps, both found the hard way, both now locked by tests

- **The MAD-trim trap.** `robustFit` discards outliers *by design*, so a long straight edge
  plus a 10px feature hanging off it still fits "straight to 0.2px RMS" — the feature having
  been trimmed — and crisping then **flattens the very thing the fit ignored**. First cut of
  edge-trace erased the strip tail exactly as line-snap had. `straightMaxDevPx` 1 now
  requires EVERY point in a run to be close, outliers included.
- **The half-pixel frame.** Mask space (`traceLoops` / `rasterizePolygons`) puts pixel
  centres at `x+0.5`; a gradient array indexes centres at integer `x`. Every artwork lookup
  now converts in one place (`edgeAlong`, `luminanceProbe`, `tensorProbe`). Note for the
  record: **`line-snap@1` does not do this conversion**, so its snapped lines sit ~0.5px
  off the ridge they matched — which is visible in the adherence table below.

### MEASURE IT: edge adherence (`measureAdherence`, `generate-masks.ts adherence`)

Marching-squares the alpha at the half level (sub-pixel, so an antialiased boundary is not
scored as if it lay on the lattice); every 0.5px, along that sample's own normal, find the
**NEAREST** local maximum ≥ floor within ±4px — *nearest*, not strongest, because a card
border has two edges and the strongest one is often not the one under the boundary;
plateaux are centred, or a step edge between two pixel centres reads as 0.5px off forever.

Probe = **line-snap's own luminance Sobel**, deliberately NOT edge-trace's colour tensor: a
tracer scored on its own edge map proves nothing, so the incumbent gets the home field.

| mask | supp% | ≤1px | ≤1px given evidence | mean dist | p95 |
|---|---|---|---|---|---|
| his hand mask (pre-line-snap) | 78.9 | 50.7% | 64.3% | 0.847px | 1.75 |
| `line-snap@1` | 78.4 | 74.8% | 95.4% | 0.545px | 1.00 |
| his correction (`ai-corrected`) | 79.2 | 75.4% | 95.2% | 0.548px | 1.00 |
| **`edge-trace@1`** | 79.0 | **77.1%** | **97.7%** | **0.199px** | 1.00 |

Colour-tensor probe (floor 24), which can see the chroma edges: his hand 68.5% / 0.746px →
line-snap 87.2% / 0.621px → his correction 92.0% / 0.564px → **edge-trace 98.4% / 0.082px**.

### Tropius me05-001/37184, run `edgetrace-tropius-1` (edge-trace@1)

317 stretches traced from the artwork (3,622px of boundary), 7 degenerate 1px stretches kept
as drawn, 12 genuinely-straight runs crisped (3,356px). Mean move 0.80px, max 2.62px.
**1,652px changed = 0.50% of the face; Jaccard vs his mask 0.9904.** Confidence 0.659.

What it traces that line-snap could not, in `local dev-artifacts/foil-shots/mask-edgetrace/`:
**A** the BASIC stage tag's slanted, rounded right end; **B** the species strip's swept tail
(line-snap squared it off; he hand-restored it; edge-trace traces it more accurately than
his own staircase); **C** the card field's corner fillets, which line-snap made hard 90°
corners by line intersection.

### Honest residuals

- **At rendered scale the difference is invisible.** 9,324 of 792,000 canvas px differ, all
  on the boundary hairline. The case for this generator is the mask, not the render.
- **Edge softness 0.419 → 0.976.** A sub-pixel contour has partial coverage on essentially
  every boundary pixel. That is more accurate than a hard-thresholded hand edge, but it is
  NOT his hand's AA character, and line-snap's "match his softness" branch no longer fires.
- **supp% drops 79.2 → 79.0** on the luminance probe: a little of the boundary now follows a
  chroma-only edge that a luminance Sobel cannot see. Real, small, and the direction you
  would expect from the operator change.
- **n = 1 card.** One demonstration, not validation. The generator has never been run on a
  card whose foil boundary is not a modern-SV reverse-holo sheet.
- The **art-window corner** (case D) is a wash — that corner's printed radius is ~1px, so
  all three masks agree to within half a pixel. Not every case is a win.

### Verification

Typecheck api + web clean, api + web builds green, `test:foil` **35/35** (10 new: the
premise, a printed rounded corner traced as an arc, the MAD-trim trap, "no printed edge ⇒
nothing moves", "no scan ⇒ refuses outright", "a strong edge outside the corridor cannot
pull the boundary", the metric measuring what it claims, sub-pixel contours, and the two
probes disagreeing about a colour-only edge). The `revert` path was exercised **twice** end
to end — `sha256sum` identical on the live mask and `git status data/` empty after each —
then re-applied. Visuals (4-way mask side-by-side + change map, four acceptance-case zoom
triptychs, rendered foil at rest + 2 tilts at desktop and 390px for both masks, a 4×
rendered diff, and the provenance panel showing `AI · UNREVIEWED` and
`me05-001/37184 · edge-trace@1 · conf 0.659` in the review queue) in
`local dev-artifacts/foil-shots/mask-edgetrace/`, with the numbers on `00-summary.png`.

**Undo, in one line:**
`pnpm --filter pokedex-api exec tsx src/foil/generate-masks.ts revert --run-id edgetrace-tropius-1`

## 2026-08-08 — Mask editor: pan + pinch-zoom, and the card stops getting "selected"
**Decided by:** user (the two asks), agent (mechanism).

**Chey, in chat, verbatim:** *"something that would be nice is to be able to pan and pinch to
zoom while editing masks. In addition, sometimes the card image gets 'selected' when I'm
trying to draw which is annoying. Fix those things now."*

He was about to hand-draw a batch of masks, so this was blocking work, not polish.

### The transform lives in the camera, not in CSS

The obvious implementation — CSS-scale the whole viewer — is wrong here: the card is a
three.js render, so scaling it just magnifies a 390px-wide framebuffer. Useless for the one
thing he zooms *for*, which is tracing a printed edge. Instead one transform `{zoom, x, y}`
(`foil/ViewTransform.tsx`) is expressed twice:

- **three.js** — `camera.setViewOffset(W·z, H·z, x, y, W, H)` renders the `(x,y,W,H)` window
  out of a *virtual* `W·z × H·z` image. The card is re-rasterized at the zoomed size, so 4×
  zoom is 4× real detail. (Verified visually at 3.4×: the "Egg Pokémon. Length: 3' 7""
  flavour line and the individual starlight sparkles are crisp, not interpolated.)
- **the overlays** — one wrapper div inside `CardViewer` carries
  `translate(-x,-y) scale(z)`, written imperatively. No React state, no re-render per frame
  — the workbench's existing `settingsRef`/tilt-target-ref ethos.

**Why that makes the pointer math free:** `setViewOffset` is a pure crop of a linearly-scaled
virtual render, so a point at base coord `u` lands at `u·z − offset` — exactly what the CSS
transform does to the overlays. `getBoundingClientRect()` on a transformed element already
reports the on-screen box, so `MaskEditor.toMask()` is **unchanged**. Verified numerically,
not by eye: a stroke drawn in the top-left corner at 3.375× lands at texels `[84,180,108,196]`
against a predicted `[85,180,109,196]` (1 screen px = 0.257 texels there); at 1× it is
`[204,323,284,350]` vs `[205,323,285,351]`. Touch at 3.0× on both 390px and 744px lands
within **1 texel**.

### The brush is constant in SCREEN px, not mask px

`lineWidth = brushSize / zoom`. He zooms to trace fine printed edges, so zooming in must buy
*finer* control; a mask-constant brush would paint a 4×-fatter-looking stroke at 4× zoom and
defeat the whole feature. At 3.375× a 28px brush is 8.3 mask texels. Documented in the UI
under the brush slider, because the number on the slider now means "at 1×".

### Gesture split (Procreate's model, because he'll already know it)

| input | in mask-edit / window-adjust |
|---|---|
| two fingers | pinch-zoom + pan, anchored on the gesture midpoint |
| one finger, *allow finger drawing* OFF | **pans** (it previously did nothing) |
| one finger, *allow finger drawing* ON | paints; pan needs two fingers |
| one finger on a window handle | drags the handle (never pans) |
| Pencil / mouse | always paints; **while a pen is down, touches are ignored** (palm) |
| wheel / trackpad pinch | zoom at the cursor · middle-drag or Space+drag pans · `+ − 0` keys |

A second finger landing mid-stroke **aborts and rolls that stroke back** (pops the undo
snapshot taken at pointerdown) — a pinch must not leave a stray dab. Verified: 349px painted
mid-stroke → 0px after the pinch, and the mask is not marked dirty by the rollback.

Pan is hard-clamped to the virtual render, so the card can never be lost off-screen; the HUD
(`− 100% + ⤢`, bottom-right, 32px thumb targets) shows the zoom and fits back in one tap.
Zoom is live only in edit/adjust mode and resets on exit — **the tilt interaction is
untouched**. `WindowEditor` got two zoom terms: drag deltas divide by zoom, and handles
counter-scale via `--foil-zoom` so they stay 44px on screen (measured 44.0 → 44.0 at 2.25×).

**Zoom never reaches the artifact.** Only the *display* is transformed; the mask canvas is
never touched. A mask painted and saved at 3.375× round-tripped from disk as 490×674 with a
pixel-identical bbox `[83,179,133,218]`, n=552 — same as the canvas.

### Fix 2 — the "selected" card

Classic selection/drag-ghost during a paint gesture. Applied on the whole viewer column and
both canvases: `user-select:none`, `-webkit-user-select:none`, `-webkit-touch-callout:none`,
`-webkit-user-drag:none`, `draggable={false}`, `preventDefault` on `dragstart`/`selectstart`
and on the paint `pointerdown`. `touch-action:none` was already right and is deliberately
**not** widened — the gestures above are pointer-event-driven, so the two fixes don't fight.
A 400×300px drag across the card and its floating labels now yields an empty
`window.getSelection()` and zero `dragstart` events.

For iOS specifically: `touch-action` alone does **not** stop Safari's page pinch-zoom, so the
host also `preventDefault`s `gesturestart`/`gesturechange`/`gestureend` and non-passive
`touchmove` while editing.

### The bug that cost the most time

Window-level listeners (`pointerup`, `keydown`, …) registered once in the controller factory
were silently killed by **StrictMode's simulated unmount** — the effect cleanup ran, nothing
re-registered them, and the app looked fine right up until the first pinch, which then
*started and never ended*: `touches` never emptied, mode stuck at `pinch`, and every
subsequent gesture AND every touch stroke was dead. Diagnosed by counting real pointer events
in the page (they were all firing) rather than by reading the handler. **Rule: any listener a
long-lived controller owns must be added in a symmetric `attach()`/`detach()` pair driven by
an effect — never in the factory.** Same class of bug made Space re-fire the last-clicked HUD
button (a `<button>` activates on key*up*), so both halves of the Space key are now prevented.

### Verification

Typecheck all workspaces clean, web build green. 44/44 browser checks
(`chromium`, desktop 1280 + touch-emulated 390 and 744 via CDP `Input.dispatchTouchEvent`):
placement at 1× and 3.375×, pinch, two-finger pan, one-finger pan/paint split, mid-stroke
rollback, wheel zoom, Space-drag, fit, save round-trip, no-selection/no-ghost, window handles
at zoom, tilt restored on exit, and **his committed masks (base1-8, base1-5, me05-001) still
load as `Hand mask (saved)`**. Shots at all three widths in
`local dev-artifacts/foil-shots/mask-ux/`. Scratch card was base1-3; its artifacts were deleted
and `data/` md5-matches its pre-run state.

**Honest limits:** `-webkit-touch-callout` cannot be verified in Chromium (Blink doesn't
implement it — computed value is empty), so the long-press-callout half of fix 2 is verified
by construction and by the iOS-only gesture-event veto, not by a test. And at high zoom a
window handle can sit outside the (now `overflow-hidden`) viewer — that's what pan is for.

## 2026-08-08 — The region policy is LEARNABLE; the subject silhouette still is not
**Decided by:** agent (measurement), pending user (two rule fixes + one review).
**Branch:** `foil/mask-learn`. **Run id:** `regionlearn-me05-1` — 8 masks, applied, reversible.

**Chey's goal, verbatim, from the outset:** *"once i've made a few hand-done masks, i want an
AI to be able to learn from mine to be able to do its best at replicating it across other
cards in similar sets/series - and then i want to be able to correct the agents' mask such
that it can then observe the diff and continue to improve without me having to hand-paint all
the masks."* He has now hand-authored 7 masks in 2 classes. This is that lane.

### What the last lane proved, and what it left missing

`edgetrace-me05-batch-1` (2026-08-08) seeded `edge-trace@1` from the era rectangle on five
me05 reverses. All five failed identically, and the diagnosis was decisive: **99.7% of the
gap between the layout rule and his intent is REGION decisions, not boundary crispness.** A
tracer crisps a boundary; it can never add or remove a region. The missing half was a thing
that decides WHICH REGIONS carry foil. That is `region-learn@1`.

### The shape of it (`apps/api/src/foil/region-learn.ts`, pure, no I/O)

1. **Partition** the card face into five structural classes computed from the card's OWN
   printing — `border` (achromatic ring reachable from outside the card), `furniture`
   (achromatic printed elements inside it), `frameBody` (the coloured field outside the
   illustration), `windowBackground` (inside the illustration, colour-connected to its own
   inner edge), `windowSubject` (inside the illustration, the rest). The era rect is a
   **search prior only**.
2. **Read the policy** off his masks: per class, what share of its pixels did he make foil?
   >50% ⇒ that class carries foil. Where his exemplars agree, the agreement IS the policy;
   where they split it is REPORTED (`RegionPolicy.disagreements`) and never averaged into a
   decision.
3. **Apply** to a new card, then hand the region boundary to `edge-trace@1` for the geometry.
   Regions from the corpus, geometry from the artwork — the division of labour the last lane's
   failure defined.

Nothing about Pokémon is hard-coded, and the proof is that **the same code reads opposite
policies from the two classes**. That is locked by a test, not asserted.

### THE POLICY, in plain words

> **`modern-sv` / `sheet`** — foil = **the coloured frame body, and nothing else**. It covers
> the name bar, HP, type icon, attack area, weakness/resistance/retreat row, flavour text,
> illustrator and set-number line, and it stops at every piece of **silver furniture** and at
> the illustration: the species strip *including its flared tails*, the BASIC/STAGE tag, the
> evolution medallion *and the coloured sprite inside it*, the "Evolves from X" bar, the
> copyright footer.
>
> **THE SILVER BORDER RING IS NOT FOIL.** It carries 0.2% / 0.2% / 0.4% of foil across his
> three sv masks — he stops at the inner edge of the coloured frame all the way round,
> rounded corners included. The batch lane flagged this as *"Chey's call, not a measurement"*.
> **It is now a measurement, and it is unanimous.**
>
> Restated physically, which is why it generalises: **the reverse foil sits under the
> coloured ink, not under the silver.** CHROMA is the signal; luminance is not.
>
> **`wotc` / `window`** — foil = **the illustration's own background, minus the subject
> silhouette, minus the stage/evolution box where it overlaps the window**. Card stock and
> yellow border carry none (0.2–0.9%). Confirmed 4/4; the stage-box clause shows on both
> evolved cards (Gyarados, Machamp) and is inapplicable on both Basics.

### THE BAR — written down before the generator produced a single number

`/tmp/…/scratchpad/BAR.md`, and now `eval --bar-mean/--bar-min`: **a class is ready for a
batch iff mean IoU ≥ 0.90 AND no held-out card is below 0.85.** Boundary distance is a
per-card diagnostic that never promotes a class — the last lane proved edge adherence
measures precision, not correctness.

### LEAVE-ONE-OUT (each card's own mask withheld from the pool it learns from)

| class | card | era rect | region-learn@1 | Δ | bnd mean / p95 |
|---|---|---|---|---|---|
| modern-sv/sheet | me05-001 Tropius | 0.7607 | **0.9803** | +0.2196 | 3.64 / 23px |
| | me05-007 Heatran | 0.7596 | **0.9752** | +0.2156 | 4.82 / 55px |
| | me05-012 Armarouge | 0.7496 | **0.9519** | +0.2023 | 3.86 / 15px |
| | **mean** | **0.7566** | **0.9691** | **+0.2125** | **PASS** |
| wotc/window | base1-7 Hitmonchan | 0.7318 | 0.9502 | +0.2184 | 2.75 / 10.7px |
| | base1-5 Clefairy | 0.5342 | 0.8995 | +0.3653 | 4.08 / 18px |
| | base1-6 Gyarados | 0.5503 | 0.8789 | +0.3286 | 2.21 / 11px |
| | base1-8 Machamp | 0.6422 | **0.8599** | +0.2177 | 6.25 / 33px |
| | **mean** | **0.6146** | **0.8971** | **+0.2825** | **FAIL by 0.0029** |

**wotc/window failed its bar and no batch was generated for it.** Not "close enough" — the
bar existed before the number did, and the number came in under it.

**The two failures are different in kind, and that is the finding.** modern-sv's residual is
a 1–2px registration hairline round the frame perimeter (me05-012's whole 7,244px shortfall
is that hairline). wotc's residual is 8,343px of **Machamp's body** read as background,
because his blue-grey sits inside the colour distance of the dark teal holo field. Every
frame-level decision on WOTC is right — box found, stage box cut out, card stock excluded.
The subject silhouette is the one region this cannot segment.

**Ceiling honesty:** the scalar params (chroma threshold, morph radius, background sigma)
were chosen while looking at the corpus. Only the region POLICY is properly held out, so
0.8971 and 0.9691 are ceilings, not floors.

### THE BEVEL SIDE — a general finding, and the single fix that rescued Gyarados

An illustration box is framed by a bevel, so its edge is **two parallel printed lines**, both
real, both strong. Which one the foil boundary sits on is not a detection question — it is
which side the foil is on. `window` scope stops at the INNER line, `sheet` at the OUTER.
"Take the strongest peak" chooses between them at random card by card: it put base1-6's
detected window top 8px wrong (onto a text-line edge at y=72), which collapsed its background
segmentation and scored it **0.0001**. Taking the foil-side line took the same card to
**0.8789**. `detectWindow(..., foilInsideWindow)` now takes the side explicitly.

### TWO ERA RECTS ARE WRONG — not changed here, they are Chey's call

`wotc.artWindow` is **11px too high at the top and 6px too wide at the right** on a 490×674
mask. His four masks put the box at top 79–81 / right 436–439; the rule says 68.7 / 443.5.
Each card's own printed edge agrees with him within 1–2px on 3 of 4. `base1-8` Machamp is the
card the era was "measured on" — mis-measured, exactly as `me04-007` was for modern-sv.
Implied: `{ x: 0.1051, y: 0.1187, w: 0.7878, h: 0.3991 }`. **Era-wide blast radius across
every Base/Jungle/Fossil/Gym/Neo/e-Card holo, so it belongs to `foil/main` with his eyes on
it.** `region-learn@1` does not need it — it detects the box per card.

### THE BATCH — `regionlearn-me05-1`, 8 cards, `ai` / unreviewed

`me05-002, -003, -005, -006, -010, -014, -024, -029` — deliberately spread across **5 frame
colours** (green, red, blue, yellow, purple) and **4 Stage-1 cards with an evolution
medallion**, including four of the five cards the previous lane failed on. Confidence
0.431–0.85. Undo:
...
**Revert proven, twice, and one bug fixed on the way.** run → revert → run: after the revert
the corpus is byte-identical to the pre-run state (73 files, every sha256 matching, `git
status data/` clean) **and so is the directory tree**. That last clause needed a fix: revert
deleted a run's files but left the empty card directory behind, so `data/foil-masks/me05-014/`
survived as a card that looks masked to anything that lists the directory (`readCorpus` walks
directories). The fix removes the directory too, and only when the run created it and nothing
else lives there. The two applies produced **byte-identical PNGs** (all 80), differing only in
the sidecars' `savedAt`/`generatedAt`.

### THE FIVE STALE TRIAL MASKS — recommend reverting

`base1-1/2/4/10/15` (run `wotc-window-trial-1`, `window-artgate@1`, `ai`, weight 0, nobody has
ever looked at them). The mission said: supersede them if the new generator passes for
wotc/window, recommend reverting if it fails. **It failed.** And the generator that made them
scores mean **0.7694** / worst **0.7330** on the same leave-one-out — far below anything this
lane would ship. They are unreviewed machine output he never asked for, cluttering the review
queue and the corpus dashboard's `meanAgreement`. Not deleted by me — his corpus, one line:

```
pnpm --filter pokedex-api exec tsx src/foil/generate-masks.ts revert --run-id wotc-window-trial-1
```

### THE SINGLE MOST VALUABLE NEXT HAND MASK

**`base1-2` Blastoise (Base Set holo, `window`).** The wotc failure is entirely "subject
colour close to background colour", and it is monotone in that closeness: Clefairy (pink on
teal) 0.90, Hitmonchan (orange on green) 0.95, Gyarados (blue on blue-teal) 0.88, Machamp
(blue-grey on teal) 0.86. Two more light-subject cards teach nothing. A blue subject on the
blue-teal Base field is the failure mode itself, and it is the mask that would let a future
lane fit a subject model against a case that actually breaks. Second choice: any Gym or Neo
holo, to test a different frame's bevel — but that is a geometry question, and geometry is
already solved.

For **modern-sv** the corpus needs breadth, not difficulty: all three exemplars are from one
set (me05). One reverse from a Scarlet & Violet main-series set would prove the policy is
about the FRAME and not about Pitch Black's print run.

### One thing in the batch he should look at first

On `me05-014` Seaking the evolution medallion does not quite reach the border ring, leaving a
narrow coloured sliver trapped between them. `region-learn` drops it (it keeps only the
largest coloured component, which is what correctly drops the sprite inside a medallion —
his me05-012 mask agrees about the sprite). **He has never ruled on the sliver.** It is an
extrapolation, it is visible in `04-zoom-policy-regions.png`, and one correction settles it.

### Verification

Typecheck api + web clean; api + web builds green; `test:foil` **46/46** (11 new in
`__tests__/region-learn.test.ts`: the same code reads opposite policies from a sheet mask and
a window mask *and the votes prove it*; border and furniture read as excluded; a learned
policy round-trips its own evidence; split exemplars are reported as disputed with both
numbers, never averaged; **unreviewed `ai` is never an exemplar — at the weight table, at
`selectExemplars`, and outnumbering the humans 3:1 changes nothing**; an `ai-corrected` mask
at weight 0.6 cannot outvote a `hand` one; the bevel side is taken on the foil's side and
lands on the printed line it claims; no evidence ⇒ the prior survives and every edge reports
REFUSED; a peak beyond `windowMaxMovePx` is refused; the five classes partition the face
exactly once; a coloured island marooned in silver furniture is not frame body).

Visuals in `local dev-artifacts/foil-shots/mask-learn/`: the numbers on `00-summary.png`, both
leave-one-out sheets (rule | his | generated | where they disagree), the batch as rule vs
generated for all 8, zoomed crops of the policy-critical regions (stage tag + medallion,
species strip, border + footer), and the live workbench on `me05-014` — rest and tilt at
desktop and 390px with the Poké Ball hologram running through the generated mask, plus the
mask-overlay frame that shows the region decision directly in the app. The `AI · UNREVIEWED`
badge is visible on every one.

---

## 2026-08-08 — Masks are VECTOR TEMPLATES, not per-card rasters; and his hand is already more vector-like than the machine
**Decided by:** user (two instructions in one conversation), implemented on `foil/mask-vector`.

**What he said**, verbatim, first on intent:
...
and then, correcting the scale of the deliverable mid-run:

> "We don't need 3,454 vector masks. All of these share the same 2 layouts really."

**Decision.** The committed artifact for a mask class is a **parameterised vector layout
template** — line segments, circular arcs, exact corners, in normalised card space — applied
at render time. Not 3,454 rasters. Per-card masks now exist only as EXCEPTIONS: a human mask,
or a card that genuinely deviates beyond tolerance. `apps/web/src/foil/vector-templates.json`,
**13.3 KB**, 58 primitives, sits beside `era-layouts.json` and is diffable and hand-correctable.

**The finding that reframes the whole lane.** Measured at 0.35 px tolerance on sub-pixel
contours, primitives needed per 1 000 px of boundary:

| | long-primitive frac | prim/kpx | residual px | axis-aligned |
|---|---|---|---|---|
| `region-learn@1` (what he was shown) | 0.9047 | **33.72** | 0.0898 | 0.7875 |
| **Chey's hand masks** | 0.9679 | **21.35** | 0.0907 | 0.8554 |
| `vector-template@1` (this lane) | 0.9861 | **10.34** | 0.0637 | 0.9321 |

**His hand masks are already more vector-like than the generator that was supposed to be
learning from them.** Where the card prints a straight edge he draws it straight; the
livewire tracer in `edge-trace@1` hugged gradient noise pixel by pixel and wobbled ±2 px
(see `zoom-window-edge-straightness.png` — his line dead straight, the machine's ragged).
So "add smoothing" was never the fix. The fix is to stop emitting a traced path at all.
**Vector-ness never promotes a class** — like boundary distance and edge adherence before it,
it describes; IoU gates.

**ONE layout, not two.** Over all 11 masks: 50.05% of the face is foil on *every* one,
47.18% bare on every one, 2.77% contested — and the Basic-vs-evolving difference is a single
2 646 px blob, the evolution medallion. The fitter **discovers** that optional element from
the corpus (open the contested set to kill the boundary band, take the largest survivor,
split the exemplars on their share inside it — an 80.5 pp gap) rather than being told a
medallion exists. That machinery is era-agnostic.

**His corrections ruled on three things** (full numbers in `data/foil-masks/codified/modern-sv.md`
pass 2). (a) **The me05-014 Seaking sliver IS foil** — the open question pass 1 explicitly
flagged as never ruled on. `region-learn@1` scored 0.0% there on all four Stage-1 cards; he
added it on all four. "Largest coloured component" was the wrong rule; the right one is
*every coloured region that reaches the border ring*. (b) **Achromatic ink printed on the
coloured field carries foil** — colourless energy symbols, retreat symbols, the regulation
box, the name text. Furniture is distinguished topologically (it touches the border or the
window), not by chroma. (c) The window edge sits on the bevel's **outer** line.

**Leave-one-out vs the bar committed in `38e12cc` before the generator existed** (mean ≥ 0.94,
none below 0.90): **0.9907 mean, 0.9843 worst — PASS**, against `region-learn@1`'s 0.9757 /
0.9519 on the same 11, with boundary p95 of 1 px on 10 of 11 (was 2–72 px). On all 10 cards
with a machine parent the template lands closer to **his corrected mask** than to the machine
version he rejected.

**Storage.** The rasterisation depends only on `(era, scope, hasOptionalElement, w, h)` — not
on the card — so every Basic reverse gets byte-identical pixels. The on-demand cache for
modern-sv/sheet is **2 PNGs (~17 KB)** under `IMAGE_CACHE_ROOT/foil-templates/`, mirroring the
image-cache contract: disk-only, outside git, safe to delete. Committing per-card masks
instead would have been ~100–350 MB. A hand mask **always** answers first; the template only
fills what used to 404, and is labelled `unreviewed` in the response headers with the
generator identity, run id and exemplar count.

**Two bugs the synthetic round-trip test caught that the corpus IoU table hid**, both now
locked: `vectorizeLoop` was emitting **raw contour points** as join vertices (so a fitted
"vertical" edge ran x=25.00→25.66 — a polyline in a vector's clothes), and the **arc sweep
sign was inverted** (`cross < 0` where y-down makes a positive cross clockwise), so every arc
bulged the wrong way. Round-trip IoU 0.9482 → 0.9994.

**Scope limits, stated rather than papered over.** Trainer (431 variants) and Energy (104)
are a *different layout* with **zero** exemplars — the template is visibly wrong on them and
the optional-element probe agrees with the catalog on 0/49 of them versus 293/328 on Pokémon.
Out of scope until he draws one of each. And pale frames cannot self-verify: adherence flags
45.5% of Lightning and 41.0% of Colorless cards against **0.0%** of Fire, Psychic and Darkness
— a contrast effect, not a geometry error (calibration: on the 11 cards where the boundary is
known correct, the same probe reads 0.720 for *his own hand masks* and 0.758 for the template).
...
His words, on hearing modern reverses resolved to one template + one optional element:
"The ones we'll need a lot of unique masks for are the ex cards. But even those will
have a few basic shared layouts with a unique added or subtracted shape along with that
basic template. But we'll get to that later."

**Recorded because the current architecture already anticipates it and the next lane
should not redesign around it.** `vector-template@1` discovers an OPTIONAL ELEMENT from
consensus (the evolution medallion blob emerged from an 80.5pp bimodal split; it was not
declared). That mechanism generalises directly to his ex model: a class resolves to a
small set of base templates, and a card carries an additive or subtractive delta shape on
top. What changes for ex cards is only the *cardinality* — deltas become per-card and
common rather than per-class and rare — so the work is (a) letting a card reference
`template + delta` instead of `template` alone, (b) fitting the delta from the card's own
artwork under the learned policy, (c) storing deltas as vector primitives (they stay tiny;
the ~97MB per-card raster problem does not come back).

Not scheduled. Sequenced after modern non-reverse holos, which he named as the next run.


## 2026-08-08 — Chey: holos = shared layout + SUBTRACTIVE silhouette delta (forward note)

His words, going into hand-masking trainer layouts and non-reverse holo variants:
"Holo non reverse ones will be trickier to learn because the shape of every Pokemon
illustration is different - this will be more like the wotc era holos. But I think this
will follow the same recipe - shared basic layout with subtractive vector that minuses
the specific Pokemon silhouette."

**Why this matters more than it looks: it reframes the one measured failure in the
project.** `region-learn@1` FAILED its leave-one-out bar on wotc/window (0.8971 vs 0.90),
and the diagnosis was unambiguous — every frame-level decision was right; the whole
residual was 8,343px of Machamp's body read as background (his blue-grey sits inside the
colour distance of the teal holo field). The error is monotone in subject-vs-background
colour distance: Hitmonchan 0.95 > Clefairy 0.90 > Gyarados 0.88 > Machamp 0.86. The lane
concluded "the subject silhouette is the one region this cannot segment" and treated that
as a ceiling.

Under his model it is not a ceiling, it is a DELTA. Consequences for the next lane:
- The base template carries the frame (already solved, transfers from modern reverse).
- The silhouette becomes a per-card SUBTRACTIVE vector delta — fitted from the artwork,
  stored as primitives, and independently correctable.
- Therefore a human correction touches ONLY the silhouette, never the frame. That is a
  large reduction in his per-card effort and it makes the correction diffs a clean
  training signal for silhouette fitting specifically (frame noise no longer contaminates
  them).
- Segmentation quality can then be measured on the delta alone, rather than being hidden
  inside a whole-mask IoU where a correct frame masks a wrong subject.

Open question the next lane must answer from his exemplars, NOT assume: for modern
non-reverse holos the bevel side flips vs reverses, and `windowSubject` vs
`windowBackground` was already flagged as live — his phrasing ("minuses the specific
Pokemon silhouette") indicates subtractive, i.e. foil in the background and NOT under the
subject. Confirm against his masks before encoding it.

Not scheduled; he is hand-masking trainer layouts + non-reverse holos now.


## 2026-08-08 — Chey CONFIRMS: modern non-reverse holos = foil in the BACKGROUND, subject subtracted

His ruling, closing the open question from the entry above: foil covers the background art,
and the Pokémon silhouette is SUBTRACTED (no foil under the subject). This is the same
physical process as WOTC-era holos — the illustration window has foil everywhere except
under the printed subject — and it means the `template + subtractive delta` architecture
already sketched above is confirmed, not speculative.

**Encoding:** `windowBackground` is the correct scope for the base template's illustration
window region. The per-card delta is a subtractive vector fitted to the subject silhouette.
A card with no delta gets foil across the entire window (wrong, but honestly labelled
`unreviewed`); corrections only ever touch the silhouette shape.
