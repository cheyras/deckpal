...
**Operational gotcha from this run, worth not repeating:** stopping a per-branch dev API with
`pkill -f "apps/api/dist/index.js"` also matches any **deployed** process running from the
same path. The process manager restarted it within seconds, but it was a real if brief prod
