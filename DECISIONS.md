...
## 2026-08-08 — Rule-seeding is UNSOUND for modern-sv/sheet; the layout rect is the bug
**Decided by:** agent (measurement), pending user (the rule fix).
**Run id:** `edgetrace-me05-batch-1` — generated, measured, **reverted**. Nothing shipped.

**Chey, in chat, verbatim:** *"Use it on the next 5 reverse holofoils in the same set."*

### The five, and why

Every me05 card after 001 in collector order that actually has a REVERSE variant, all five
with art in the cache and none with any existing mask:

| card | variant | who | why chosen |
|---|---|---|---|
| me05-002 | 37186 | Grubbin | Basic, green frame |
| me05-003 | 37188 | Fomantis | Basic, green frame |
| me05-005 | 37191 | Poltchageist | Basic, green frame |
| me05-006 | 37193 | Sinistcha | **Stage 1** — evolution medallion overlaps the art window |
| me05-007 | 37195 | Heatran | Basic, **red** frame (different chroma) |

**Skipped: me05-004 Lurantis ex** — Double rare, `holo:37189` only, no reverse variant. (Same
for 008 Mega Delphox ex and 016 Wailord ex further down; ex cards are holo-only.)

### The problem the mission named, and it is worse than "the corridor is too tight"

On Tropius, `edge-trace` was seeded by Chey's own boundary. These five have no hand mask, so
the seed has to be the resolver's rule (`era-layouts.json` `modern-sv.artWindow`, scope
`sheet`, invert). **Two structural facts make that a different problem, not a harder one:**

1. **`edge-trace` crisps a BOUNDARY; it can never add or remove a REGION.** Whatever the rule
   over- or under-claims survives the run untouched, at any parameter setting.
2. **There were zero exemplars to fall back on.** `selectExemplars` gives `modern-sv/sheet`
   **0** masks right now: the only one is Tropius, and the `edge-trace` proposal sitting on
   top of his `ai-corrected` mask makes it weight-0 `ai`. The anti-collapse floor blocks every
   generator on this era+scope until Chey reverts or reviews that proposal. Working as designed
   — and worth knowing, because it means "learn the regions from his exemplar" is not
   available either, without touching his pending review.

### THE CONTROL (step 3): rule-seeded trace vs his known intent, on Tropius itself

Dry, in a scratchpad; the live me05-001 mask was never written to.

| mask | IoU vs his `ai-corrected` | boundary mean | p95 |
|---|---|---|---|
| era rectangle, untraced | 0.7084 | 79.1px | 336 |
| era rect → `edge-trace@1` **defaults** | **0.7080** | 44.8px | 149 |
| … `corridorPx` 12 / 24 / 48 | 0.7079 / 0.7079 / 0.7080 | — | — |
| … `anchorSnapPx` 8 | 0.7077 | 44.9px | 149 |
| … `anchorSnapPx` 20 (`corridorPx` 24) | 0.8250 | 9.6px | 40 |
| … `anchorSnapPx` 40 | 0.7621 (`supp%` 51.8 — collapse) | 12.5px | 41 |
| his window rect + `anchorSnapPx` 20 | 0.8921 | 4.9px | 21 |
| **his own hand mask** | **1.0000** | 0 | 0 |
| the live `edge-trace` refine OF his mask | 0.9904 | 0.57px | 1.19 |

**Rule-seeded edge-trace closed ZERO of the gap** (0.7084 → 0.7080). Widening `corridorPx`
did **nothing at all** — 5/12/24/48 produce the same 90 traced / 218 kept, because the binding
constraint is `anchorSnapPx` (3), not the corridor. My going-in assumption was wrong and the
measurement said so.

`anchorSnapPx` 20 is the knob that moves, and it is not safe: the outer boundary starts hopping
between the silver border's two parallel edges and takes excursions clean off the card
(`09b-control-widened-snap20-zoom-excursions.png`). That is precisely the relocate-vs-crisp
failure `proximityPower` and a small snap radius exist to prevent. There is no wide setting
that is both better and stable.

### WHERE the gap lives — and why no tracer reaches it

Rule XOR his mask = **70,520px**, in three connected regions, and **all of it is the rule
over-claiming** (he never claimed foil the rule didn't, bar a 211px antialiasing hairline):

| px | region | rule | Chey |
|---|---|---|---|
| 44,039 | the silver border ring + the square image corners | FOIL | not foil |
| 26,270 | the 41px window-top band + the species strip + the BASIC stage tag | FOIL | not foil |
| 211 | a 1px AA hairline at the window's left edge | not foil | FOIL |

**99.7% of the gap is region decisions.** `edge-trace` addresses boundary precision. It is the
right tool pointed at the wrong 0.3%.

### THE REAL BUG: `modern-sv.artWindow.h = 0.315` is wrong, on every card

Chey's hand-adjusted window for Tropius (recorded in the archived sidecar's `prior.window`)
is `h = 0.3749417758369723` — the top edge at y=66.1px instead of y=106.5px. **That is not a
Tropius preference, it is a measurement error in the era layout**, and six cards say so
independently of his mask. Strongest horizontal tensor ridge in y∈[40,140], summed across the
window's x span:

| card | printed window top | era rect (0.315) | his rect (0.3749) |
|---|---|---|---|
| me05-001 | y=65 | y=106.5 ✗ | y=66.1 ✓ |
| me05-002 | y=65 | ✗ | ✓ |
| me05-003 | y=65 | ✗ | ✓ |
| me05-005 | y=65 | ✗ | ✓ |
| me05-006 | y=62/66 (Stage 1 medallion splits it) | ✗ | ✓ |
| me05-007 | y=65 | ✗ | ✓ |

Adherence of the bare rectangle against each card's own printed edges (tensor probe, floor 24):
`supp%` **71.8–89.7 → 94.9–100**, `within1px` **31.4–41.6% → 42.8–62.1%**. The era rect is
~41px short at the top for the whole SV/Mega era, and `me04-007 (Chesnaught)`, the card it was
"measured on", is not special — it is just mis-measured.

**Not changed here.** `era-layouts.json` drives the live resolver for every Scarlet & Violet and
Mega Evolution card, holo and reverse. That is a one-line data change with era-wide blast
radius and it belongs to `foil/main` with Chey's eyes on it, not to a mask batch.

### THE BATCH (step 4): generated, measured, and reverted

`--seed rule` was built properly (below) and run on all five under one run id. Per card, with
the seed being the era rectangle and the target its own scan:

| card | changed px | % face | bnd μ | p95 | adherence `within1px` seed→traced | mean dist | conf | traced/kept | **traced window top y** |
|---|---|---|---|---|---|---|---|---|---|
| me05-002 Grubbin | 1,971 | 0.60% | 8.61 | 3.6 | 53.8 → **84.7%** | 1.015 → 0.398px | 0.083 | 110/198 | **106** (true 65) |
| me05-003 Fomantis | 1,584 | 0.48% | 9.18 | 3.1 | 38.9 → **62.2%** | 1.129 → 0.622px | 0.051 | 93/215 | **106.3** |
| me05-005 Poltchageist | 1,479 | 0.45% | 9.44 | 2.6 | 39.4 → **69.9%** | 0.949 → 0.207px | 0.082 | 83/225 | **106.0** |
| me05-006 Sinistcha | 1,627 | 0.49% | 9.06 | 2.4 | 39.1 → **70.7%** | 1.007 → 0.264px | 0.081 | 90/218 | **106.4** |
| me05-007 Heatran | 1,611 | 0.49% | 9.44 | 2.6 | 39.4 → **73.0%** | 0.940 → 0.200px | 0.084 | 82/226 | **106.2** |

**Every adherence number improved and every mask is wrong.** 1,068 of ~1,080 "kept" stretches
are the outer loop, refused for the right reason ("no printed edge under the source boundary")
— because the rule's outer bound is the **image rectangle**, over the silver border and past
the card's rounded corners, where there is no edge to find.

**All five failed, in the same way, for the same reason. No card passed.** This is not "worked
on three": it is 0/5, and the failure mode is the seed, not the tracer.

`03-FAILURE-foil-over-the-illustration-grubbin-tilt.png` is the whole argument in one frame:
at tilt, the foil sheen runs across the top of the illustration and stops at a ragged line
that follows the *leaves*, because `edge-trace` faithfully traced the artwork's own edges.

### THE TRAP THIS FOUND: edge adherence measures PRECISION, not CORRECTNESS

An illustration is full of real edges. A rule line dropped inside one will always find an edge
to sit on, score beautifully, and be 41px from the boundary anybody meant. The metric that
settled `line-snap` vs `edge-trace` (both seeded by his intent, so only precision was in
question) is **not** an approval signal when the seed itself is in question. Two new tests lock
this so the next agent hits a failing assertion instead of a plausible mask.

Same caution for the corpus dashboard: `meanAgreement` is agreement with the *rule*, so the
batch pushed `modern-sv` to 0.9459 — a rule-seeded batch flatters that number by construction.

### What was actually built (kept — the capability is sound, the seeding is not)

- **`--seed rule`** in `generate-masks.ts`: the era rectangle as a refiner's source, for cards
  with no mask. Labels itself relentlessly — `exemplars: []` (learned from nobody),
  `params.seed = 'layout-rule'`, `params.source = '<era>/<scope> layout rectangle'`,
  confidence **halved**, notes opening `RULE-SEEDED` and naming the region limit. Skips
  `minExemplars` deliberately (that floor guards *learning*; a deterministic rect has no pool
  to collapse) and refuses any card that already has a mask.
- **`--param k=v,k=v`** per-run generator overrides, merged into the RECORDED identity params
  so a mask always carries the values that actually made it; `edgeTraceMask` reports its
  effective params back.
- **`--dump <dir>`** on a dry run: proposal PNG + seed PNG + report.json, so review screenshots
  come out of the real code path instead of a re-implementation.
- **Report wording made seed-neutral** in `edge-trace.ts` ("the source boundary", "kept exactly
  as given"). "His stroke … as drawn" on a rule-seeded mask is a lie told by a template.
- **`revert` bug fixed:** it removed a run's freshly-created masks but left the card
  directories behind, so the corpus was not byte-for-byte back to where it started.

### Verification

Typecheck api + web clean; api + web builds green; `test:foil` **39/39** (4 new: a rule seed
40px off is not rescued by any `corridorPx`; adherence rises while the mask stays wrong; params
are overridable and reported verbatim; a rule-seeded generator run announces itself and halves
its confidence). Run→revert→run→revert exercised end to end: after each revert the corpus is
**byte-identical** to the pre-run state (53 files, 14 dirs, every sha256 matching) and
`git status data/` is clean; the two applies produced **byte-identical mask PNGs**, differing
only in the sidecars' `savedAt`/`generatedAt` — deterministic pixels, timestamped records.
Visuals in `~/.deckscout-dev/foil-shots/mask-edgetrace-batch/` (summary sheet with the
numbers, five overlays + five change maps, the control triptych and the widened-snap failure,
five zooms, rendered foil at rest + tilt for all five at desktop and for two at 390px).

**Final state: nothing applied.** The corpus is exactly as it was.

**To materialise the batch and see it yourself (one line, and one line to undo):**
```
pnpm --filter pokedex-api exec tsx src/foil/generate-masks.ts run --generator edge-trace \
  --seed rule --era modern-sv --scope sheet --serie me --series-slug mega-evolution \
  --run-id edgetrace-me05-batch-1 \
  --cards me05-002:37186,me05-003:37188,me05-005:37191,me05-006:37193,me05-007:37195
pnpm --filter pokedex-api exec tsx src/foil/generate-masks.ts revert --run-id edgetrace-me05-batch-1
```

### What would actually make the next five work, in order

1. **Fix `modern-sv.artWindow`** to `h ≈ 0.3749` (`y ≈ 0.0981` top-left). Six cards agree; his
   hand adjustment agrees; it fixes a live rendering error on every SV/Mega holo and reverse,
   not just the masks. Needs Chey's OK — era-wide blast radius.
2. **Give the `sheet` scope an outer bound that is the card, not the image.** Its boundary is
   currently the texture rectangle; every SV reverse holo therefore foils past the card's
   rounded corners. Whether the *silver border* is inside or outside the foil is Chey's call,
   not a measurement — that one needs him.
3. **Then one more hand mask**, on a card of a *different* layout (a Stage 1 like me05-006, or
   a non-green frame like me05-007), to teach the two region decisions the rule cannot hold:
   the species strip and the stage tag. With ≥1 human exemplar back in the pool, seeding a
   refiner from an *exemplar* rather than a rectangle becomes available — and that is the
   version of this batch worth running.
4. **Reverting or reviewing `edgetrace-tropius-1`** is the prerequisite for (3): while that
   proposal is live, `modern-sv/sheet` has zero eligible exemplars.
