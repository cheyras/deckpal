...
What it traces that line-snap could not, in `~/.deckscout-dev/foil-shots/mask-edgetrace/`:
...
`~/.deckscout-dev/foil-shots/mask-edgetrace/`, with the numbers on `00-summary.png`.
...
`~/.deckscout-dev/foil-shots/mask-ux/`. Scratch card was base1-3; its artifacts were deleted
...
Visuals in `~/.deckscout-dev/foil-shots/mask-learn/`: the numbers on `00-summary.png`, both
...
`pkill -f "apps/api/dist/index.js"` also matches the **deployed** `deckscout-api` process, which runs
from `apps/api/dist/index.js`. the process manager restarted it within seconds (prod
