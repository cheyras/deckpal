// deckscout — pm2 process definitions (Phase 7 deploy artifact).
//
...
// /home/cheyras/pokedex/.env via @deckscout/db's loadEnv() at process start. loadEnv()
// resolves .env relative to the @deckscout/db module (repo root), so it works regardless
...
// The apps read DECKSCOUT_API_PORT / DECKSCOUT_IMAGES_PORT (not $PORT) — verified in
