# DeckScout — deployment runbook (Phase 7)
...
- Built all backend apps (`@deckscout/db`, `deckscout-api`, `deckscout-images`, `deckscout-sync`)
  and the production frontend. `deckscout-sync` had **no `dist/` before** and now builds
  cleanly; the other three already had `dist/` and were rebuilt to be current.
- Frontend: `apps/web/dist` (644 KB), base `/deckscout/` baked into `index.html`
  (`/deckscout/assets/index-*.js`). JS 116 KB gzip.
- Smoke-tested the two servers on localhost (then stopped them, ports freed):
  `GET /deckscout/api/health` → 200, `/deckscout/api/series` → 200,
  `/deckscout/images/en/base/base1/004/high.webp` → 200 image/webp. The old
  `/api/deckscout/...` path → 404, confirming the previous draft fragment was wrong.

The superseded draft `deploy/nginx/thegrid-deckscout.conf` was **removed** in this commit:
it used the wrong API prefix (`/api/deckscout/`) and proxied `/deckscout/` to :3700 for the
...
rtk psql -U deckscout -h 127.0.0.1 -d deckscout -c 'select 1'      # DB reachable with .env creds
rtk nginx -t                                   # current config is valid BEFORE we touch it
```

---

## Stage A — pm2 (deckscout-api, deckscout-images, deckscout-sync)

Runs as its **own** app list; it is **not** merged into thegrid-api's ecosystem.

**Apply**
```bash
rtk pm2 start /home/cheyras/pokedex/deploy/ecosystem.deckscout.cjs
rtk pm2 save                                   # persist across reboot (updates the dump)
```

**Verify**
```bash
rtk pm2 list                                   # deckscout-api / -images / -sync all "online"
rtk ss -tln | grep -E ':3700|:3701'            # both now LISTEN on 127.0.0.1
rtk curl -s http://127.0.0.1:3700/deckscout/api/health          # {"status":"ok","db":"up",...}
rtk curl -s http://127.0.0.1:3701/api/deckscout/images/health   # {"status":"ok",...}
rtk pm2 logs deckscout-sync --lines 20 --nostream               # "deckscout-sync up. sync_run rows: N"
# Existing services untouched — connections stay within budget:
rtk psql -U deckscout -h 127.0.0.1 -d deckscout -c \
  "select application_name, count(*) from pg_stat_activity where usename='deckscout' group by 1"
# expect deckscout-api ≤2, deckscout-sync ≤1  (hard cap 3 total)
```

**Rollback**
```bash
rtk pm2 delete deckscout-api deckscout-images deckscout-sync
rtk pm2 save
...
nginx worker (`www-data`) cannot traverse into it — the `/deckscout/` `alias` block would
...
- Alternative if ACLs are undesirable: copy the build to a path nginx already reads
  ...
  and change the `alias` in both nginx fragments to `/var/www/deckscout/`. That trades the
...
```
include /home/cheyras/pokedex/deploy/nginx-thegrid-deckscout.conf;
```
...
```
include /home/cheyras/pokedex/deploy/nginx-brain-public-deckscout.conf;
```
...
**Verify deckscout + that nothing else regressed**
```bash
# deckscout over LAN (plaintext):
rtk curl -s -H 'Host: thegrid' http://127.0.0.1/deckscout/api/health         # {"status":"ok"...}
rtk curl -sI -H 'Host: thegrid' http://127.0.0.1/deckscout/ | head -1        # 200
rtk curl -s  -H 'Host: thegrid' http://127.0.0.1/deckscout/set/base1 | grep -o '/deckscout/assets/[^"]*' | head -1
#   ^ deep client route must return the SPA shell (index.html referencing /deckscout/assets/…),
#     NOT a 404 — this is the alias+try_files check.
rtk curl -sI -H 'Host: thegrid' http://127.0.0.1/deckscout/assets/ 2>/dev/null # a real asset → 200
...
# Public vhost: deckscout should 302 to the Authelia portal when unauthenticated (gate works):
rtk curl -sI https://cheyrasnet.tplinkdns.com/deckscout/ | grep -iE 'HTTP/|location'
#   expect 302 → /authelia/?rd=…   (same as /git/ and /lumina/)
```
Then load `https://cheyrasnet.tplinkdns.com/deckscout/` in a real browser (log in via
...
**Apply**
```bash
rtk sudo cp /home/cheyras/pokedex/deploy/dnsmasq-deckscout.conf /etc/dnsmasq.d/deckscout.conf
...
# From a LAN client, https://cheyrasnet.tplinkdns.com/deckscout/ should load with a VALID cert
...
**Rollback**
```bash
rtk sudo rm /etc/dnsmasq.d/deckscout.conf
...
1. **SPA `alias` + `try_files` (moderate confidence).** The directives are the standard,
   alias-safe subpath form: the fallback `/deckscout/index.html` is an absolute URI, so it
   does an internal redirect that re-enters `location /deckscout/` and resolves via the alias
   ...
   `/deckscout/set/base1` curl: if that returns the app shell, the pattern is correct on
   this nginx build. If it 404s, switch the fallback to a named location:
   `location @deckscout_spa { rewrite ^ /deckscout/index.html break; root /home/cheyras/pokedex/apps/web/dist; }`
   and `try_files $uri $uri/ @deckscout_spa;` — but the current form is expected to work.
...
3. **Authelia turns 401 into a 302 to the portal**, including for `/deckscout/api/*` XHR and
