...
5. **Embedding infra already exists on this box.** pgvector 0.8.0 is available in the cluster (the `openbrain` DB uses it with HNSW); `nomic-embed-text` is in local ollama (OpenAI-compatible `/v1/embeddings`). "Same approach as OpenBrain" is literal: `CREATE EXTENSION vector` in the pokedex DB + ollama embeddings. No new infra, no GPU.
...
- `CREATE EXTENSION vector` in the pokedex DB.
...
Wave boundaries are also **checkpoint boundaries** (source roadmap rule, kept): at each one, write a short retro to OpenBrain — what shipped, what the next wave's real cost looks like — and get Chey's explicit go. The five-project shape of this roadmap is exactly the over-engineering pattern he watches for; the checkpoint is the guardrail.
