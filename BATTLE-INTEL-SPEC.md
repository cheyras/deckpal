...
- **Battle intel is chat-first.** Everything ships as DeckPal MCP tools consumed from claude.ai / Claude Code. Pokédex UI surfaces for intel (deck battle tab, matchup views) are a later, separate feature — do not bundle them into A-track branches.
...
- `CREATE EXTENSION vector` in the deckpal DB.
