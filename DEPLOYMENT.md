...
| `LABELER_ENTITLED_USER_IDS` | `<uuid>,<uuid>` | **Who may open `/dev/quad-labeler` and write quad labels, beyond the owner.** Comma-separated `auth.users` UUIDs. Guards two things that must agree: the route in `apps/web/src/main.tsx` (which reads the server-computed `labeler` flag on `GET /me`) and `POST /dev/scan-flags` (`labelerOnlyInProduction`), which is the only path a saved label takes — gating one and not the other gives an entitled account a surface it can open and cannot save from. **Optional, and normally unset.** When it is unset or empty, `DECKE_ENTITLED_USER_IDS` is inherited, because that list already names the owner plus the QA account and the training surface wants exactly those two; the 2026-09-08 ruling was for the surface to be live for both without a config step. Setting this variable REPLACES the inherited list — that is how the two features get decoupled if Deck-E is ever opened wider. `GET /health` reports `labelerEntitlement` as `owner-plus-list` (dedicated), `owner-plus-decke-list` (inherited), `owner-only`, `nobody` or `self-host` — the status only, never the ids, for the reason the row above gives. **`/scan` itself does NOT widen**: the scanner stays `ownerOnlyInProduction('not-found')` per the 2026-09-07 ruling. |
