...
**Ingress.** A `location` block on the existing `thegrid` vhost (LAN) and on
`brain-public` behind Authelia (remote), per the user's locked decision. The app
...
the build. `ColorSplash/deploy.sh` already builds on-Pi in production today.

### 9.1 🔴 PWA is currently impossible on LAN

`http://the.grid/` is **not a secure context** — the LAN vhost is plaintext
`listen 80`. No service worker, no install, no offline, on any browser. Only
`cheyrasnet.tplinkdns.com` has TLS. The brief requires PWA and offline resilience,
so **this must be resolved or the requirement dropped**.

Cheapest fix: **split-horizon DNS**. dnsmasq is already running on this box, so one
`address=/cheyrasnet.tplinkdns.com/<pi-lan-ip>` line makes the existing Let's
