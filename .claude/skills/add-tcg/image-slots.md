...
`pnpm --filter pokedex-images manifest:check` (exits non-zero on drift) before declaring it done.
...
- Game-specific: no.
- Added: (original build) · notes: feeds the scanner — after warming, `scan:index` + restart `pokedex-api`. 2026-08-07: 1,970 files here had no manifest row; backfilled, and the ad-hoc scripts that caused it were folded into `warmGaps.ts`/`warmFromPkmn.ts`.
