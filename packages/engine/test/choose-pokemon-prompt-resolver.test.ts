...
import { State, Player, ResolvePromptAction, GameMessage, ChoosePokemonPrompt,
  PokemonCard, PokemonSlot, PlayerType, SlotType } from '../src/common';
import { ChoosePokemonPromptResolver } from '../src/bot/prompt-resolver/choose-pokemon-prompt-resolver';
import {
  allSimpleTactics,
  allPromptResolvers,
  defaultStateScores,
...
describe('ChoosePokemonPromptResolver', () => {
  ...
  it('Should choose the strongest Pokemon', () => {
    ...
    assert.deepStrictEqual(result, [player.bench[2]]);
  });
  ...
  it('Should choose the weakest Pokemon for the opponent', () => {
    ...
    // then
    assert.ok(action instanceof ResolvePromptAction);
    assert.deepStrictEqual(result, [opponent.bench[2]]);
  });
  ...
  it('Should not choose blocked Pokemon', () => {
    ...
    assert.ok(result.includes(player.bench[0]));
    assert.ok(result.includes(player.bench[2]));
  });
