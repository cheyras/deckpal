...
import { State, Player, ResolvePromptAction, GameMessage, PokemonSlot,
  PutDamagePrompt, PlayerType, SlotType, PokemonCard, DamageMap} from '../src/common';
import { PutDamagePromptResolver } from '../src/bot/prompt-resolver/put-damage-prompt-resolver';
import {
  allSimpleTactics,
  allPromptResolvers,
  defaultStateScores,
...
describe('PutDamagePromptResolver', () => {
  ...
  it('Should put damage on Bench, because Active is blocked', () => {
    ...
    // then
    assert.ok(action instanceof ResolvePromptAction);
    assert.deepStrictEqual(result[0].target.player, PlayerType.TOP_PLAYER);
    assert.deepStrictEqual(result[0].target.slot, SlotType.BENCH);
    assert.deepStrictEqual(result[0].damage, 20);
  });
