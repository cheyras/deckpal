import { Card } from '../../../common';
import { AerodactylEx } from './aerodactyl-ex';
import { AggronEx } from './aggron-ex';
import { Anorith } from './anorith';
import { Anorith2 } from './anorith-2';
import { Arbok } from './arbok';
import { Arcanine } from './arcanine';
import { Armaldo } from './armaldo';
import { Aron } from './aron';
import { Azumarill } from './azumarill';
import { Azurill } from './azurill';
import { Baltoy } from './baltoy';
import { Breloom } from './breloom';
import { Cacnea } from './cacnea';
import { Cacnea2 } from './cacnea-2';
import { Cacturne } from './cacturne';
import { ClawFossil } from './claw-fossil';
import { Cradily } from './cradily';
import { Cyndaquil } from './cyndaquil';
import { Delcatty } from './delcatty';
import { DoubleFullHeal } from './double-full-heal';
import { Dunsparce } from './dunsparce';
import { Dusclops } from './dusclops';
import { Duskull } from './duskull';
import { Duskull2 } from './duskull-2';
import { Eevee } from './eevee';
import { Ekans } from './ekans';
import { Electabuzz } from './electabuzz';
import { Elekid } from './elekid';
import { Espeon } from './espeon';
import { Fearow } from './fearow';
import { Flareon } from './flareon';
import { GardevoirEx } from './gardevoir-ex';
import { Golduck } from './golduck';
import { Growlithe } from './growlithe';
import { Illumise } from './illumise';
import { Jolteon } from './jolteon';
import { Kabuto } from './kabuto';
import { KabutopsEx } from './kabutops-ex';
import { Kecleon } from './kecleon';
import { Kirlia } from './kirlia';
import { Lairon } from './lairon';
import { LanettesNetSearch } from './lanettes-net-search';
import { Lileep } from './lileep';
import { Lileep2 } from './lileep-2';
import { Linoone } from './linoone';
import { Lombre } from './lombre';
import { Lombre2 } from './lombre-2';
import { Lotad } from './lotad';
import { Lotad2 } from './lotad-2';
import { Ludicolo } from './ludicolo';
import { Lunatone } from './lunatone';
import { Marill } from './marill';
import { Mawile } from './mawile';
import { MultiEnergy } from './multi-energy';
import { Murkrow } from './murkrow';
import { MysteriousFossil } from './mysterious-fossil';
import { Natu } from './natu';
import { Nuzleaf } from './nuzleaf';
import { Nuzleaf2 } from './nuzleaf-2';
import { Omanyte } from './omanyte';
import { Omastar } from './omastar';
import { Onix } from './onix';
import { Pelipper } from './pelipper';
import { Pichu } from './pichu';
import { Pikachu } from './pikachu';
import { Psyduck } from './psyduck';
import { Quilava } from './quilava';
import { RaichuEx } from './raichu-ex';
import { Ralts } from './ralts';
import { RareCandy } from './rare-candy';
import { RootFossil } from './root-fossil';
import { Sableye } from './sableye';
import { Sandshrew } from './sandshrew';
import { Sandslash } from './sandslash';
import { Seedot } from './seedot';
import { Seedot2 } from './seedot-2';
import { Seviper } from './seviper';
import { Shiftry } from './shiftry';
import { Shiftry2 } from './shiftry-2';
import { Shroomish } from './shroomish';
import { Skitty } from './skitty';
import { Slakoth } from './slakoth';
import { Solrock } from './solrock';
import { Spearow } from './spearow';
import { Steelix } from './steelix';
import { Trapinch } from './trapinch';
import { TyphlosionEx } from './typhlosion-ex';
import { Umbreon } from './umbreon';
import { Vaporeon } from './vaporeon';
import { Vigoroth } from './vigoroth';
import { Volbeat } from './volbeat';
import { Wailmer } from './wailmer';
import { WailordEx } from './wailord-ex';
import { WallysTraining } from './wallys-training';
import { Wingull } from './wingull';
import { Wobbuffet } from './wobbuffet';
import { Wynaut } from './wynaut';
import { Xatu } from './xatu';
import { Zangoose } from './zangoose';
import { Zigzagoon } from './zigzagoon';

export const setSandstorm: Card[] = [
  new AerodactylEx(),
  new AggronEx(),
  new Anorith(),
  new Anorith2(),
  new Arbok(),
  new Arcanine(),
  new Armaldo(),
  new Aron(),
  new Azumarill(),
  new Azurill(),
  new Baltoy(),
  new Breloom(),
  new Cacnea(),
  new Cacnea2(),
  new Cacturne(),
  new ClawFossil(),
  new Cradily(),
  new Cyndaquil(),
  new Delcatty(),
  new DoubleFullHeal(),
  new Dunsparce(),
  new Dusclops(),
  new Duskull(),
  new Duskull2(),
  new Eevee(),
  new Ekans(),
  new Electabuzz(),
  new Elekid(),
  new Espeon(),
  new Fearow(),
  new Flareon(),
  new GardevoirEx(),
  new Golduck(),
  new Growlithe(),
  new Illumise(),
  new Jolteon(),
  new Kabuto(),
  new KabutopsEx(),
  new Kecleon(),
  new Kirlia(),
  new Lairon(),
  new LanettesNetSearch(),
  new Lileep(),
  new Lileep2(),
  new Linoone(),
  new Lombre(),
  new Lombre2(),
  new Lotad(),
  new Lotad2(),
  new Ludicolo(),
  new Lunatone(),
  new Marill(),
  new Mawile(),
  new MultiEnergy(),
  new Murkrow(),
  new MysteriousFossil(),
  new Natu(),
  new Nuzleaf(),
  new Nuzleaf2(),
  new Omanyte(),
  new Omastar(),
  new Onix(),
  new Pelipper(),
  new Pichu(),
  new Pikachu(),
  new Psyduck(),
  new Quilava(),
  new RaichuEx(),
  new Ralts(),
  new RareCandy(),
  new RootFossil(),
  new Sableye(),
  new Sandshrew(),
  new Sandslash(),
  new Seedot(),
  new Seedot2(),
  new Seviper(),
  new Shiftry(),
  new Shiftry2(),
  new Shroomish(),
  new Skitty(),
  new Slakoth(),
  new Solrock(),
  new Spearow(),
  new Steelix(),
  new Trapinch(),
  new TyphlosionEx(),
  new Umbreon(),
  new Vaporeon(),
  new Vigoroth(),
  new Volbeat(),
  new Wailmer(),
  new WailordEx(),
  new WallysTraining(),
  new Wingull(),
  new Wobbuffet(),
  new Wynaut(),
  new Xatu(),
  new Zangoose(),
  new Zigzagoon(),
];
