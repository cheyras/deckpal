...
/**
 * The tensor shape `embedInput` produces, as an ONNX runtime wants it:
 * 1 x 3 x SIZE x SIZE, NCHW.
 *
 * Declared HERE rather than beside whichever session happens to run the model,
 * because there is more than one caller and they have to agree: the API's
 * server-side embedder (`apps/api/src/scan/queryEmbed.ts`) and the catalogue
 * job's Python worker feed the same graph. ORT accepts any dims that multiply
 * out to the right length and will produce confident nonsense from a transposed
 * image, so the shape is pinned against the spec that fills it rather than
 * written out as a literal in each place that needs one.
 */
export const EMBED_INPUT_DIMS: readonly number[] = [1, 3, EMBED_SIZE, EMBED_SIZE]
