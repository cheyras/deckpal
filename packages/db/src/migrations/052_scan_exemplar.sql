-- 052 · The verify flywheel: what a confirmed scan leaves behind.
--
-- ══════════════════════════════════════════════════════════════════════════════
...
--
-- `resolved_by` and `phash_distance` are the two columns the 2026-09-06 ruling
-- adds, and they exist to answer ONE question the owner reserved: whether the
-- perceptual hash is ever the reason an answer was right.
--
--   The ruling demotes phash to "a near-exact fast path + telemetry column",
--   retired "only if the accuracy benchmark shows it never changes an
--   outcome". A benchmark cannot show that unless every verified scan records
--   BOTH which rung answered and what the hash thought at the time. Derive
--   either one later and the answer is a guess; store both now and
--   "of the scans phash corroborated, how many would the other signals have
--   got right alone" is a GROUP BY.
--
--   `phash_distance` is NULLABLE and null is the common case, because the
--   ruling also moves phash off the capture path: a scan that never asked the
--   hash has no distance, and 64 would be a lie meaning "maximally
--   dissimilar" rather than "not consulted".
--
--   `resolved_by` is TEXT with NO check constraint, unlike `sleeve` two
--   sections down, and the asymmetry is deliberate. `sleeve` is a closed
--   physical vocabulary the reader picks from; `resolved_by` is the ladder's
--   own rung names (`apps/api/src/scan/resolve.ts`), and that ladder is
--   append-only by design — rung 9 was added after rung 8 shipped. A CHECK
--   here would mean a migration every time the ladder grows a rung, and the
--   failure it would prevent (a typo'd rung name) is visible in the very
--   GROUP BY this column exists for.
--
-- ══════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE scan_exemplar (
  ...
  -- WHICH RUNG CLAIMED IT: the ladder's own `resolvedBy`, verbatim
  -- ('badge+number', 'number+denominator', 'name+number', 'family-text',
  -- 'vector', 'corroborated', 'prior-only'). NULL only for a scan recorded
  -- before the ladder ran at all. No CHECK — see the header.
  resolved_by         TEXT,
  -- WHAT THE HASH THOUGHT, if it was asked. NULL is the common case and means
  -- "not consulted", which is a different fact from a distance of 64. 0..64.
  ...
  -- The range of a 64-bit Hamming distance, and the same bound POST /scan's
  -- own body validation applies. A value outside it did not come from the hash.
  CONSTRAINT scan_exemplar_phash_distance_range
    CHECK (phash_distance IS NULL OR phash_distance BETWEEN 0 AND 64),
...
-- The calibration read: of the scans the gate called confident, BY WHICH RUNG,
-- which were corrected. Partial, because the interesting rows are the confident
-- ones and indexing the rest would triple it for no query.
--
-- `resolved_by` is in the key rather than a second index because the question
-- the 2026-09-06 ruling actually reserved is per-rung — "does the hash ever
-- change an outcome" is `WHERE identity_level='confident' GROUP BY resolved_by,
-- corrected`, and a rung-blind precision figure cannot answer it.
CREATE INDEX scan_exemplar_calibration_idx
  ON scan_exemplar (identity_level, resolved_by, corrected)
...
CREATE TABLE scan_exemplar_frame (
  ...
  -- 🔴 768, THE SAME WIDTH AS card_embedding (051), AND THAT IS THE WHOLE
  -- POINT OF THE COLUMN. This started life as `vector(384)` — ViTamin-S's
  -- feature width, the checkpoint that won the spike's FIRST gallery and then
  -- stopped separating when the gallery doubled (embed-spike/NOTES.md §5). The
  -- shipped checkpoint is 768-wide, and a flywheel storing query vectors in a
  -- space the catalogue's vectors do not live in cannot be compared to
  -- anything: no cosine, no "which catalogue card was this closest to", no
  -- calibration. It would have accumulated for months and been worth nothing,
  -- silently, which is exactly the failure `stamp` exists to prevent one layer
  -- up. The width is pinned to `EMBED_DIM` in packages/matching, same as 051,
  -- and a checkpoint of another width is a new migration for BOTH tables.
...
COMMENT ON COLUMN scan_exemplar.resolved_by IS
  'The ladder rung that claimed this card, verbatim from resolve.ts: '
  'badge+number | number+denominator | name+number | family-text | vector | '
  'corroborated | prior-only. Deliberately unconstrained — the ladder is '
  'append-only and a CHECK would need a migration per rung.';

COMMENT ON COLUMN scan_exemplar.phash_distance IS
  'What the perceptual hash thought, 0..64, or NULL for "not consulted" — the '
  'common case now that phash is off the capture path. This is the telemetry '
  'half of the 2026-09-06 ruling that demoted the hash: it retires when this '
  'column shows it never changed an outcome, and not before.';

COMMENT ON COLUMN scan_exemplar_frame.embedding IS
  'A QUERY embedding in the same 768-d space as card_embedding (051). Same '
  'width is a correctness requirement, not a coincidence: a stored query '
  'vector that cannot be compared to the catalogue is not evidence of '
  'anything.';
