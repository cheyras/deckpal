...
Data lives in host **Postgres**, database `deckscout`. Runs behind nginx at `http://the.grid/deckscout/`
(LAN) and an Authelia-gated `https://cheyrasnet.tplinkdns.com/deckscout/` (remote). Deployed via
...
- **Git:** commits go on `main`; upstream is the local Gitea —
  `origin http://localhost:3000/cheyras/deckscout.git` (browse at `http://the.grid/git/cheyras/deckscout`).
...
`deckscout-devhub` (`tools/devhub/`); Chey reviews worktree UI from his phone through it. A
