...
to all ports, the router only forwards 80/443). Runs under pm2 as `deckscout-dev-hub-legacy`.
...
- **`POST /register`** — how a surface joins the menu. Registry persists at
  `~/.deckscout-dev-hub-legacy/surfaces.json` (outside the repo; survives restarts).
...
The hub lists dev surfaces only — no prod entry (removed 2026-08-01; it broke for raw-IP visitors and the hub's job is in-flight work). Reach the live app directly at `http://localhost/deckscout/` when needed.
