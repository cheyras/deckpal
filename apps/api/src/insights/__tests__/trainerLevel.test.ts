...
// ── Trainer Level: floor(unique / 10), level-0 start ─────────────────────────
...
// ── set LVL from percentage: 0 if 0 else 1 + floor(pct/25) ───────────────────────
...
test('pct — one-decimal round-half-up, the one rounding rule every % uses', () => {
