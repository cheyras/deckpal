...
test('assertQueryVector accepts a well-formed unit vector', () => {
  const v = assertQueryVector(vec(1));
  assert.equal(v.length, EMBED_DIM);
});

test('assertQueryVector rejects the wrong width before Postgres has to', () => {
  // A wrong-width vector reaches pgvector as a cast error naming neither the
  // field nor the expected size. Since the server now produces the vector
  // itself, this width can only be wrong if SCAN_EMBED_MODEL_PATH points at a
  // checkpoint that is not the one the catalogue was embedded with — which
  // otherwise surfaces as a plausible ranking of the wrong thing.
  assert.throws(() => assertQueryVector(new Float32Array(128)), /must have 768 components, got 128/);
});

test('assertQueryVector rejects a non-finite component', () => {
  const bad = vec(2);
  bad[7] = Number.NaN;
  assert.throws(() => assertQueryVector(bad), /query embedding\[7\] is not a finite number/);
});

test('assertQueryVector refuses an un-normalised vector instead of fixing it', () => {
  // Silently re-normalising would hide a producer that is not following the
  // input spec, and its vectors are wrong in every OTHER place they land
  // whatever this function does with them.
  const scaled = vec(3).map((x) => x * 2) as Float32Array;
  assert.throws(() => assertQueryVector(scaled), /must be L2-normalised/);
});
