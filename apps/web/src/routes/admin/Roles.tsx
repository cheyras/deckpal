...
function RoleEditor({ role, clone, catalog, ceilings, close }: { role?: AdminRole; clone: boolean; catalog: Permission[]; ceilings: Record<string, string[]>; close: () => void }) {
  ...
  const [permissions, setPermissions] = useState((role?.permissions ?? []).filter(key => role && !clone || (ceilings[String(role && role.tier <= 40 ? role.tier : 10)] ?? []).includes(key)))
  const [tier, setTier] = useState(role && role.tier <= 40 ? role.tier : 10)
  const editable = role && !clone ? role.editablePermissions : ceilings[String(tier)] ?? []
  const groups = [...new Set(catalog.map(p => p.group))]
  return <Sheet title={role && !clone ? 'Edit role' : 'Create role'} size="lg" onClose={() => { if (state.busy) return false; close() }}><form className="space-y-[16px]" onSubmit={e => { e.preventDefault(); const body = { name: name.trim(), description: description.trim(), permissions }; void state.save(() => role && !clone ? api.adminUpdateRole(role.id, { ...body, expectedRevision: role.revision }) : api.adminCreateRole({ ...body, tier }), () => { close(); invalidateAccess() }) }}>
    <Field label="Role name" value={name} onChange={e => setName(e.target.value)} required maxLength={80} />
    {(!role || clone) && <label className="block font-semibold text-text-secondary">Role tier<select aria-label="Role tier" className={selectClass + ' mt-[6px]'} value={tier} onChange={e => { setTier(Number(e.target.value)); setPermissions(current => current.filter(key => (ceilings[e.target.value] ?? []).includes(key))) }}><option value={10}>User</option><option value={20}>Superuser</option><option value={30}>Contributor</option><option value={40}>Admin</option></select></label>}
    <Field label="Description" value={description} onChange={e => setDescription(e.target.value)} maxLength={500} />
    <p className="text-[14px] text-text-muted">Choose from permissions enforced by the app. Administration access opens this workspace; each section also requires its own permission. Role tiers set authority ceilings. Contributors have Dev tools access only. Protected role identity and tier cannot be changed here.</p>
    {groups.map(group => <fieldset key={group} className="space-y-[12px] rounded-[12px] border border-border-default p-[16px]"><legend className="px-[6px] font-semibold text-text-primary">{group}</legend>{catalog.filter(p => p.group === group).map(permission => <label key={permission.key} className="flex items-start gap-[12px]"><input className="mt-[4px]" type="checkbox" checked={permissions.includes(permission.key)} disabled={!editable.includes(permission.key)} onChange={e => setPermissions(e.target.checked ? [...permissions, permission.key] : permissions.filter(p => p !== permission.key))} /><span className="min-w-0"><span className="break-words font-semibold text-text-primary">{permission.key}</span><span className="mt-[2px] block text-[14px] text-text-muted">{permission.description}</span></span></label>)}</fieldset>)}
...
export function AdminRoles() {
  const access = useAccess(), canManage = access.actorCapabilities.canEditRoles
  ...
  return <section className="min-w-0 space-y-[20px]"><div className="flex flex-wrap items-center justify-between gap-[12px]"><h2 className="font-display text-[24px] text-text-primary">Roles</h2>{canManage && <Button onClick={() => setEditing({ clone: false })}>Create role</Button>}</div>
    <p className="max-w-[760px] text-text-muted">Assign roles from a user's account. Each person has one role. Built-in role identities cannot be deleted; allowed permission definitions can be edited separately. Only Superadmins and Owners manage role definitions.</p>
    <DataTable label="Administration roles" rows={filtered.slice(offset, offset + pageSize)} getRowId={role => role.id} getRowLabel={role => role.name}
      ...
      columns={[
        ...
        { id: 'type', header: 'Type', cell: role => role.system ? 'Built-in' : 'Custom' },
        ...
        ...(canManage ? [{ id: 'actions', header: 'Actions', className: 'min-w-[260px]', cell: (role: AdminRole) => <div className="flex flex-wrap gap-[8px]">{role.canEdit && <Button variant="ghost" size="sm" aria-label={'Edit ' + role.name} onClick={() => setEditing({ role, clone: false })}>Edit</Button>}<Button variant="ghost" size="sm" aria-label={'Clone ' + role.name} onClick={() => setEditing({ role, clone: true })}>Clone</Button>{role.canDelete && <Button variant="ghost" size="sm" aria-label={'Delete ' + role.name} disabled={role.memberCount > 0} title={role.memberCount ? 'Remove assignments before deleting this role.' : undefined} onClick={() => setDeleting(role)}>Delete</Button>}</div> }] : []),
      ]}
    ...
    {canManage && editing && query.data && <RoleEditor {...editing} catalog={query.data.permissions} ceilings={query.data.permissionCeilings ?? {}} close={() => setEditing(null)} />}
