 ...
 * and no runtime image tooling. Served on a miss with a NON-immutable, no-store
 * cache header so the real art is picked up once warmed. The read path never
 * proxies upstream (ARCHITECTURE §7.5) — a miss is instant and local.
 */
