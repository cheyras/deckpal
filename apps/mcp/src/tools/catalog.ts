...
export function registerCatalogTools(server: McpServer, ctx: Ctx): void {
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        const lines: string[] = [`${card.name} | ${card.tcgdexId} | ${card.setName} #${card.localId} (${card.setTcgdexId})`];
        if (core) {
          lines.push(
            row(
              core.category,
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        const lines: string[] = [
          `${set.name} (${set.tid})${set.released_on ? ` — released ${set.released_on}` : ''}`,
        ];
