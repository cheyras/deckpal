 ...
 * goes through pokedex-api via ctx.api — the LOCKED versioning semantics
...
export function registerDeckIntelTools(server: McpServer, ctx: Ctx): void {
  server.registerTool(
    'deck_strategy',
    {
  ...
  server.registerTool(
    ...
    async ({ deck_id, log, result, player_name, opponent_deck, notes, played_at }) => {
      try {
  ...
  server.registerTool(
    ...
    async ({ deck_id, log_id, version, include_raw, page, page_size }) => {
      try {
        // ── Detail mode ───────────────────────────────────────────────────────
        if (log_id !== undefined) {
  ...
  server.registerTool(
    ...
    async ({ deck_id, version, revert_to, include_strategy, note, dry_run }) => {
      try {
