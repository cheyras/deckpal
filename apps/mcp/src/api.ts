...
/**
 * Thin fetch client for pokedex-api (SPEC §3: writes and all deck/list
...
const DEFAULT_BASE = 'http://127.0.0.1:3700/pokedex/api';

export function apiBase(): string {
  loadEnv();
  return process.env.POKEDEX_API_BASE ?? DEFAULT_BASE;
}
...
async function request(method: string, path: string, body?: unknown): Promise<unknown> {
  ...
  try {
    ...
    await sleep(500); // pokedex-api may be mid-restart under pm2; one retry only.
    res = await fetch(url, init);
  }
  ...
  if (!res.ok) {
    ...
    const message = envelope.error?.message ?? `pokedex-api ${method} ${path} → ${res.status}`;
    throw new Error(message);
  }
  return json;
}

/** GET a JSON payload from pokedex-api. `path` starts with '/', e.g. '/health'. */
export function apiGet(path: string): Promise<unknown> {
  return request('GET', path);
}

/** Send a JSON body (POST/PATCH/DELETE …) to pokedex-api. */
