...
The first pass was written before `DATA-LAYER.md` and `DECK-FORMATS.md` existed, and flagged its
...
| C7 | `price_observation` at ~45,000 variants → 2.2 GB/yr | Real priceable rows **31,610** (`DATA-LAYER.md` §4.5). Revised figures and an adopted **hybrid cadence**. | §11 |
| C8 | `observed_on DATE`, yearly partitions, PK-only index | Adopted `DATA-LAYER.md` §7.3's **`captured_at` = the source's own stamp**, **monthly** partitions, and a **BRIN** companion index. My DATE grain was a cruder way to get the same dedupe. | §7.2 |
...
**`DATA-LAYER.md` §3.1 and §8 item 12 are wrong about `cardCount`.** They state that
...
|---|---|---|
| `research/DATA-LAYER.md` | ✅ read (791 lines) | §7, §11, §15 reconciled. Divergences justified inline. |
| `research/DECK-FORMATS.md` | ✅ read (1,554 lines) | §8 rewritten. |
| `research/INTERACTION-CAPTURE.md` + `UI-SPEC.md` §5 | ✅ read | §14.2 binder verified against measured layouts. |
...
6. **Surrogate `BIGINT` PKs with `UNIQUE` natural keys, and keep the foreign keys.**
   [E] `PRIOR-ART.md` §3 item 10 — pokecollector had to *drop* `cards.set_id`'s FK to make a
...
`PRIOR-ART.md` §1 and §4 name four structural defects. Explicitly, one by one:

| pokecollector defect | Citation | How pokedex avoids it |
|---|---|---|
| **Four booleans on the card row** (`variants_normal/reverse/holo/first_edition`), README: "Variants are now limited to Normal, Holo, Reverse Holo, and First Edition" | `PRIOR-ART.md` §1 reason 1, `models.py` L113–116 | Variants are **rows** (`card_variant`, §4). A card may carry 1–8+ of them. Adding `Master Ball Pattern` or a 2027 stamp is an `INSERT`, never DDL. The four booleans cannot even be expressed. |
| **Free-text `collection.variant TEXT NOT NULL DEFAULT 'Normal'`, no FK to any variant table** | `PRIOR-ART.md` §1, §4 item 4 | `collection_item.card_variant_id BIGINT NOT NULL REFERENCES card_variant(id)`. There is no text variant column anywhere on a user row. A typo is a constraint violation, not a silently-orphaned collection entry. |
| **`price_history UNIQUE(card_id, date)` with five EUR-only columns — no variant, no currency, no source dimension** | `PRIOR-ART.md` §1, §4 item 5 | `price_observation` is keyed `(card_variant_id, source_code, currency_code, captured_at)` with `currency_code` and `source_code` as **FK'd, first-class dimensions** (§7). Cardmarket EUR and TCGplayer USD coexist as separate rows, never as separate columns. |
| **Column names that lie** — `price_market` = `price_mid` = `cardmarket.avg`; `price_high` = `avg30`; and `*-holo` meaning *reverse* holo | `PRIOR-ART.md` §4 item 6, §2a "The `-holo` semantics", `DECISIONS.md` correction 5 | Metric names are source-generic and the source→metric mapping is a **data table** (`price_source_field_map`, §7.3) rather than a line of Python. No column named `*_holo` exists anywhere. The Cardmarket `*-holo` fields are ingested onto the **Reverse Holofoil** `card_variant` row, and the mapping rows say so in SQL you can `SELECT`. |

Two more from the do-not-copy list, since they touch the schema:

| Defect | Citation | Avoided by |
|---|---|---|
| `ImageCache` as uncapped Postgres `BYTEA` — no TTL, no LRU, no size cap; `pg_dump` then carries the whole image corpus | `PRIOR-ART.md` §4 item 1 | `image_asset` (§15) holds **metadata and a filesystem path**. Bytes never enter the DB. Cap and LRU are columns, not hopes. |
| Migration strategy: `alembic` declared but unused; 108 hand-ordered raw SQL strings in `database.py` | `PRIOR-ART.md` §4 item 2 | Out of scope for this document (I write no migrations), but the DDL below is written to be expressible as ordered, reversible migrations. Nothing here depends on `CREATE ... IF NOT EXISTS` idempotency-by-reboot. |
...
And [E] `PRIOR-ART.md` §1 establishes what TCGdex actually ships: a `variants_detailed` array per
...
| Option | Why not |
|---|---|
| **A. Free-text column on the collection row** (pokecollector) | No referential integrity, no tier, unjoinable to price, typos become phantom variants. `PRIOR-ART.md` §4 item 4. Rejected. |
...
`holo/1999-2000-copyright` — into four distinct `variant_kind.code` values. [E] `DEX-DATA.md` §A.1.
...
**Validation:** the sweep independently reproduces `DATA-LAYER.md` §3.6's figures exactly —
...
[E] `DATA-LAYER.md` §4.5 measures the join coverage, and it is not total:
...
[E] `DATA-LAYER.md` §4.5: *"15 % of the corpus with no price is acceptable for a personal
...
CREATE TABLE card_set (
  ...
  -- This contradicts DATA-LAYER.md §3.1 and §8 item 12 — see the revision log at the top. (C10)
...
[E] `PRIOR-ART.md` §3 item 2 (`is_valid_price`) and §2b (pokecollect's "never write 0 for a missing
...
CREATE TABLE price_observation (
  -- ⚠ CORRECTION C8 — three changes adopted from DATA-LAYER.md §4.6 / §7.3 over my first pass:
...
when the observation actually happened — which matters because [E] `DATA-LAYER.md` §4.4 corrects the
...
*Monthly over yearly partitions.* [E] `DATA-LAYER.md` §4.6: monthly keeps the per-partition PK
...
date X", which the snapshot job and the top-movers surface both need. [E] `DATA-LAYER.md` §4.6.
...
([E] `PRIOR-ART.md` §4 item 5), not the width. Here all three dimensions are FK'd columns in the key.
...
**(c) Daily grain comes from the upstream, not from us.** [E] `DATA-LAYER.md` §4.4 corrects the
...
[E] `DECISIONS.md` correction 5 and `PRIOR-ART.md` §2a, verified live on `swsh3-136`: that card has
...
INSERT INTO price_source_field_map VALUES
 ...
 ('tcgdex-cardmarket','avg-holo',  'reverse','avg',
  ...
  'DECISIONS.md correction 5; PRIOR-ART.md §2a. Reading this literally ships wrong prices.'),
...
3. **A regression fixture pinned in the schema's own test data:** `swsh3-136` must ingest with a
   ...
   [E] `PRIOR-ART.md` §3 item 3 — *"Pin it with `swsh3-136`. Highest correctness-per-minute item on
   this list."*
...
also [E] `DEX-DATA.md` §D.2 rule 3 — *"a 'Need' row with quantity 0 must not capture."*
...
The measurement that decides it is [E] `DEX-DATA.md` §D.3, benchmarked in SQLite **on this exact
...
- **The `/series` index, `/profile`, and the dashboard render progress for every set at once**
  ...
  **≈140 ms in SQLite [P]**, and `DEX-DATA.md` §F.1 warns Postgres is *"likely 1.5–3× the SQLite
...
my arithmetic on a different query shape and a different engine. `DEX-DATA.md` §F.1 explicitly says
...
Adopted essentially as designed in [E] `DEX-DATA.md` §D.1, with the changes noted.
...
Three non-negotiable data rules, carried verbatim from [E] `DEX-DATA.md` §D.2:
...
**This is the 13-card manual-override table** [E] `DEX-DATA.md` §A.4-F6 — `Caterpie → dexId 251
...
cannot clobber the review. `DEX-DATA.md` §A.4-F6: *"Do not silently prefer one source — flag and
default to `dexId`."*
...
**Note what this table is NOT.** `DEX-DATA.md` §D.1 proposed it with `unique_cards` / `total_copies`
...
**Sprites are a pure function, not rows.** [E] `DEX-DATA.md` §D.1 —
...
|---|---|---|---|
| EN cards | 21,058 | **23,444** | [E] `DATA-LAYER.md` §3.6; reproduced by my sweep (§4.5) |
| Variants per card | ~2.1 **[P]** | **1.521** | §4.5 — my estimate was **38 % high** |
| `card_variant` rows | ≈45,000 **[P]** | **35,648** | §4.5 |
| **priceable** variant rows | — | **31,610** (88.7 %) | [E] `DATA-LAYER.md` §4.5 — 11.3 % have no third-party id (§4.6) |
...
variants. ≈780 MB/yr.** [E] `DATA-LAYER.md` §4.6.
...
- The request budget is the same either way and is not the constraint: [E] `DATA-LAYER.md` §7.2
...
- ≈780 MB/yr against ~65 GB free is a decade of headroom, and [E] `DATA-LAYER.md` §8 item 6 measures
...
**Not adopted: delta/change-only writes.** [E] `DATA-LAYER.md` §4.6 flags it as attractive but
...
1. **`PARTITION BY RANGE (captured_at)`, one partition per month** (C8). Retention becomes
   ...
   missing partition fails loudly. [E] `DATA-LAYER.md` §4.6.
...
5. **Do not put the image cache in the DB.** [E] `PRIOR-ART.md` §4 item 1 — pokecollector's BYTEA
...
| I4 | `card_species (dex_id)` | **Q7** species page ("all cards for that Pokémon"); **Q8** dex grid | `DEX-DATA.md` §D.3 — the EXPLAIN shows this index in use; measured |
...
| I16 | `price_observation` **BRIN** `(captured_at)` per partition | **Q21** "all prices captured on date X" — the snapshot job and top-movers | `DATA-LAYER.md` §4.6 |
| I17 | `sync_run (job, started_at DESC) WHERE status='ok'` | **Q22** skip-if-unchanged + `/health` | `DATA-LAYER.md` §7.3, §7.5 |
...
| I14 | `card_variant (tcgdex_variant_id)` — the UNIQUE | **Q19** catalog-sync idempotent upsert | `PRIOR-ART.md` §3 item 1 |
...
[E] `PRIOR-ART.md` §5(a): pokecollector's `BinderCard` has **no page and no slot column** and
...
`INTERACTION-CAPTURE.md` §5 and `UI-SPEC.md` measured the real binder, which is more complicated
...
[E] `PRIOR-ART.md` §3 item 1 — *"sorts deterministically (oldest attempt first, then
...
choice is mine; the requirement for cap + eviction is `PRIOR-ART.md` §4 item 1 and `DECISIONS.md`.
...
`preserve_existing_prices_for_invalid_update` ([E] `PRIOR-ART.md` §3 item 2) expressed in SQL: an
...
`holo` variant at all. [E] `PRIOR-ART.md` §2a, `DECISIONS.md` correction 5.
...
[E] `PRIOR-ART.md` §5(b) calls this *"the single largest genuine differentiator available"* and
...
`playable_fingerprint` is what makes this correct across reprints — [E] `PRIOR-ART.md` §3 item 4:
