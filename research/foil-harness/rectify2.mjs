...
export async function rectify(file) {
  ...
  // FOCUS, NOT BRIGHTNESS. Thresholding on luminance merged the card with the
  // hand holding it and the lit table behind it, and the merged blob's corners
  // are not the card's corners. But the reader is photographing the card, so the
  // card is what the lens is focused on and the background is not: local texture
  // energy separates them cleanly where brightness cannot. The hand is in focus
  // too but is smooth, so it contributes almost nothing here.
  const g = small.data
  ...
  // Box-blur the energy so a texture FIELD forms rather than isolated edges.
  const R = 7
  ...
  let mx = 0
  for (let i = 0; i < W * H; i++) if (blur[i] > mx) mx = blur[i]
  const hist = new Uint32Array(256)
  const q8 = new Uint8Array(W * H)
  for (let i = 0; i < W * H; i++) { q8[i] = Math.min(255, (blur[i] / mx * 255) | 0); hist[q8[i]]++ }
  const thr = otsu(hist, W * H)
  const mask = new Uint8Array(W * H)
  for (let i = 0; i < W * H; i++) mask[i] = q8[i] > thr ? 1 : 0
...
if (process.argv[1].endsWith('rectify2.mjs')) {
  mkdirSync(`${DIR}/rect2`, { recursive: true })
  ...
  for (const f of files) {
    ...
    if (!r) continue
    await sharp(r.buf, { raw: { width: OUT_W, height: OUT_H, channels: 3 } }).toFile(`${DIR}/rect2/${f}`)
    ok++
  }
  writeFileSync(`${DIR}/rectboxes2.json`, JSON.stringify(report, null, 2))
  const bySeg = {}
  for (const f of files) {
